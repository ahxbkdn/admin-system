...
def get_chapters():
    start_time = time.time()
    chapter_list = get_chapters_cache()
    end_time = time.time()
    print(f"GET /api/chapters 响应时间: {end_time - start_time:.4f}秒")
    return jsonify({"code": 200, "message": "操作成功", "data": chapter_list})
...
def update_section(section_id):
    ...
    if section_id in sections:
        ...
        if "videoUrl" in data:
            section["videoUrl"] = data.get("videoUrl")
        clear_cache()  # 清除缓存
        end_time = time.time()
...
def init_data():
    """初始化默认数据"""
    global chapter_id_counter, category_id_counter, question_id_counter, course_id_counter, section_id_counter
    # 初始化课程数据
    if len(courses) == 0:
        # 添加5条课程数据
        courses[1] = {
        ...
        courses[3] = {
            ...
            "description": "深入理解TypeScript的高级特性和最佳实践",
            "lecturer": "张老师",
            "totalHours": 25,
            "price": 149.0
        }
    ...
    if len(sections) == 0:
        ...
        sections[1] = {
            "id": 1,
            "chapter_id": 1,
            "title": "DevEcoStudio安装",
            "content": "DevEcoStudio的安装和配置步骤...",
            "order": 1,
            "isFree": True
        }
        sections[2] = {
    ...
    if len(questions) == 0:
        ...
        questions[1] = {
            ...
            "tags": ["React", "前端", "DOM"],
            "difficulty": 2,
            "content": "虚拟DOM是React中的一个概念，它是真实DOM的轻量级副本...",
            "status": 1
        }
        questions[2] = {
            "id": 2,
            ...
            "tags": ["React", "性能优化", "前端"],
            "difficulty": 3,
            "content": "可以通过使用React.memo、useCallback、useMemo等方式来优化React应用的性能...",
            "status": 1
        }
        questions[3] = {
            "id": 3,
